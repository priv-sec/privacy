...
def get_mia_accuracy(model_class,
    ...
    if not load_shadow:
        ...
        train_shadow_dataset, test_shadow_dataset = torch.utils.data.random_split(shadow_dataset, [half]*2)
        ...
        optimizer = optimizer(shadow_model.parameters(), **optimizer_args)
        for epoch in trange(1, target_epochs + 1):
            train(shadow_model, device, train_shadow_loader, optimizer)
        torch.save(train_shadow_dataset, "./shadow_train_news.pt")
        torch.save(test_shadow_dataset, "./shadow_test_news.pt")
        torch.save(shadow_model.state_dict(), "./shadow_model_news.pt")
    else:
        shadow_model.load_state_dict(torch.load("./shadow_model_news.pt"))
        train_shadow_dataset = torch.load("./shadow_train_news.pt")
        test_shadow_dataset = torch.load("./shadow_test_news.pt")
        train_shadow_loader = torch.utils.data.DataLoader(train_shadow_dataset,**train_kwargs)
        test_shadow_loader = torch.utils.data.DataLoader(test_shadow_dataset, **train_kwargs)
    ...
    labels_IN = np.ones(len(train_shadow_loader.dataset))

    # OUT
    preds_OUT = predict_( shadow_model, device, test_shadow_loader )
    labels_OUT = np.zeros(len(test_shadow_loader.dataset))
    ...
    return (correct / (len(train_target_loader.dataset)+len(test_target_loader.dataset))) * 100